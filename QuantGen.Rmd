...
Esse material contem minhas anotações da disciplina de Genética Quantitativa (LGN5810) ministrada pelo professor Antonio Augusto Franco Garcia na ESALQ/USP.
...
$\sigma^2(g_m) + \sigma^2(g_f)$ é a variância total e nesse caso é a variância genética aditiva.
...
No artigo o exemplo tenta explicar o custo de manutenção de tratores em função com a idade dos tratores (x).
...
Quando eu faço um polinômio diretamente com os valores de X eu espero que haja uma multicolinearidade na matriz X, o que causa um problema numérico ao inverter a matriz. Então troca-se as colunas da matriz por coeficientes ortogonais (independentes).

**Com coeficientes ortogonais**

![](imagens/img22.png)

```{r}
X1 <- c(-3,-3,-3,-1,-1,-1,1,1,5,5)
X2 <- c(178,178,178,-132,-132,-132,-237,-237,168,168)
X3 <- c(-6,-6,-6,16,16,16,-18,-18,3,3)
(X_matrix2 <- cbind(X1, X2, X3))
...
- $w_1$ e $w_2$ são contrastes ortogonais.
...
$$\mbox{Linear} \left \{ \begin{matrix}
w_1aa  = -(2f_2 + f_1) = -(2 \frac{1}{4} + 0.5) = -1 \\ 
w_1Aa = f_0 - f_2 = 0.25 - 0.25 = 0 \\ 
w_1AA = 2f_0 + f_1 = 2 \ . \ 0.25 + 0.5 = 1 
\end{matrix} \right.$$
...
w1_aa <- -1   ;   w1_Aa <-  0   ;   w1_AA <-  1
w2_aa <- -1/2 ;   w2_Aa <-  1/2 ;  w2_AA <- -1/2

X_add <- c(-1,-1,0,0,0,0,1,1)
X_dom <- c(-1/2,-1/2,1/2,1/2,1/2,1/2,-1/2,-1/2)

(X_F2 <- cbind(X_add,X_dom))
...
1, -1, -1/2, -1, -1/2),
nrow=9,ncol=5,byrow=TRUE)
X2 <- cbind(X2,X2[,2]*X2[,4],X2[,2]*X2[,5],X2[,3]*X2[,4],X2[,3]*X2[,5])
```
...
## Tarefa
- Read Cockerham (1954) (at least until page 865) and Kao & Zeng (2002). Present an overview about what you understand (30 lines).
...
[Demostração Cockerham](./tarefas/Cockerham.pdf)
...
- Read chapter 7 from Lynch & Walsh (1998) until page 139.

- (Optional) Read Jacquard (1972).

- Explain the content of slides 12 for an undergraduated student that is seeing it for the first time (30 lines).

### Minha Resposta

- Explain the content of slides 12 for an undergraduated student that is seeing it for the first time (30 lines).

_Entender o grau de relacionamento entre indivíduos é válido, através dele pois é possível estimar indiretamente fenótipos que não estão sendo observados. Para alcançar esse grau primeiramente é preciso estabelecer um período de tempo de referência, pois, devido ao ancestral comum, se voltarmos muitas gerações algum grau de parentesco os indivíduos irão possuir. Podemos utilizar três medidas:_

_- Identidade por descendência (IBD): alelos são cópias provenientes de um ancestral comum;_

_- Identidade por estado (IBS): alelos que são idênticos porém não são provenientes de um ancestral comum;_

_- Endogamia: par de alelos IBD em um indivíduo._

_Considerando dois indivíduos (x e y) e um gene, seus 4 alelos possuem 15 possibilidades de relacionamento, que foram descritas por Jacquard (1972) - Δ1 à Δ9. A partir das descrições de Jacquard, temos um modelo generalizado para calcular o Coeficiente de Coancestria (Θxy) de qualquer população, que é uma medida de probabilidade ao se tomar um gene ao acaso de dois indivíduos eles serem IBD. Dada por:_                                                                                                                                    _Θxy = Δ1 + 1/2(Δ3 + Δ5 + Δ7) + 1/4Δ8_ 

_Em que Δ é a probabilidade de dado estado de identidade. E quanto maior o Θxy, mais aparentados os indivíduos são._

_No slide 10, são apresentados 3 casos específicos e o seu coeficiente de coancestria._

_No primeiro caso, a medida é entre parentais e sua progênie. Nesse caso, o único estado de identidade possível é o Δ8. Seja x o parental e y, a progênie e que eles não são endogâmicos, já se exclui os Δ1 ao Δ6. O Δ7 é excluído pois o parental só pode passar 1 dos alelos à progênie, e nesse estado 2 são IBD. O Δ9 é excluído, pois pelo menos 1 alelo deve ser passado pela progênie, e nesse estado nenhum é IBD. Assim sobra apenas o estado Δ8.  Dessa forma, a probabilidade se ser meio será 1 e o Θxy  0,25._

_O segundo caso, os indivíduos x e y são irmãos completos. Nesse caso, eles podem compartilhar por IBD 2 alelos provenientes dos parentais (Δ7), 1 alelo (Δ8), ou nenhum (Δ9)._

![Genótipos possíveis](imagens/img31.png){height="200px" width="250px"}

_Dado o indivíduo x ter um alelo, a probabilidade do y ter o mesmo é 0,5, assim, a probabilidade dos indivíduos possuírem o alelo maternal e o paternal iguais é 0,25. Dessa forma, Δ7 será 0,25._

![Δ7](imagens/img32.png){height="200px" width="250px"}

_Para o Δ8 a probabilidade é dado o indivíduo x ter um alelo o indivíduo y ter o mesmo, ou seja, é 0,5._

![Δ8 para irmãos completos.](imagens/img33.png){height="200px" width="250px"}

_E para o Δ9 a probabilidade será a a probabilidade dos indivíduos possuírem os alelos maternal e o paternal diferentes, ou seja, 0,25. Assim, aplicando o modelo geral temos que Θxy é 0,25._

![Δ9 para irmãos completos.](imagens/img34.png){height="200px" width="250px"}

_No terceiro caso, os indivíduos são meio-irmãos, dessa forma, há a possibilidade deles compartilharem 1 alelo IBD (Δ8) ou nenhum (Δ9). Para o Δ9, seu valor será a probabilidade do alelo parental compartilhado ser diferente, ou seja, 0,5. Assim, aplicando o modelo geral temos que Θxy é 0,125._

![Δ9 para meio-irmãos.](imagens/img35.png){height="200px" width="250px"}
...
$$\Theta_{xy} = \frac{1}{4} + \frac{1}{4} = \frac{1}{2}$$

- Com endogamia ($A_1$, $A_2$):

$$\Theta_{xy} = \frac{1}{4} + \frac{1}{4}f_x + \frac{1}{4} + \frac{1}{4}f_x = \frac{1}{2}(1+f_x)$$

**Pais e Filhos:**
$$\Theta_{po} = \frac{1}{4} (1 + f_p + 2f_o)$$

**Irmãos Completos(m: mãe, f: pai):**
...
Probabilidade de um genótipo de loco único de dois indivíduos sejam ibd (probabilidade que dois idivíduos sejam ibd's pro genótipo):

_Notação:_ mães de x e y são $m_{x}$ e $m_{y}$, pais são $f_{x}$ e $f_{y}$
...
A única forma da esperaça não ser 0 é se o alelo i for o mesmo nos indivíduos x e y, se isso ocorrer, então:

$$E[(\alpha_i^x + \alpha_i^y) = E(\alpha_i^2)$$

(metade da variância genética aditiva)

E a probabilidade de i ser o mesmo é $\Theta_{xy}$, dessa forma temos a esperança como:

$$4\Theta_{xy}E(\alpha_i^2)$$

(Duas vezes a variância genética aditiva para o loco vezes $\Theta_{xy}$)

_para todos os locus:_
$$\sigma_A(x,y) = 2\Theta_{xy}\sigma_A^2$$
...
$$\sigma_{AA}(x,y) = (2\Theta_{xy})^2 \sigma_{AA}^2$$

$$\therefore \ \sigma_G(x,y) = 2\Theta_{xy}\sigma_{A}^2 + \Delta_{xy} \sigma_{D}^2 + (2\Theta_{xy})^2 \sigma_{AA}^2 + 2\Theta_{xy} \Delta_{xy} \sigma_{AD}^2 + \Delta_{xy}^2 \sigma_{DD}^2$$
...
| Relação          | $\sigma_{A}^{2}$ | $\sigma_{D}^{2}$ | $\sigma_{AA}^{2}$ | $\sigma_{AD}^{2}$ | $\sigma_{DD}^{2}$ |
|------------------|------------------|------------------|-------------------|-------------------|-------------------|
| Pais e filhos    | $\frac{1}{2}$    |                  | $\frac{1}{4}$     |                   |                   |
| Meios-irmãos     | $\frac{1}{4}$    |                  | $\frac{1}{16}$    |                   |                   |
| Irmãos completos | $\frac{1}{2}$    | $\frac{1}{4}$    | $\frac{1}{4}$     | $\frac{1}{8}$     | $\frac{1}{16}$    |
...
## Introdução
Abordagem tradicional baseada apenas em dados fenotípicos para estudar populações, arquitetura gênica, definir estratégias de melhoramento.

**Expectativa para média de linhas de cruzamento**

- duas populações parentais: $P_1$ e $P_2$;

- Cada loco em equilíbrio de HW e equilíbrio de fase gamética;

- As populações não são ligadas;

- $F_1$: cruzamento das linhas $P_1$ e $P_2$;

- $F_1$: cruzamento entre indivíduos $F_1$.

**Definições:**

**S** - fração de genes $P_1$ em uma linha;

**H** - probabilidade que um membro de uma linha tenha um $P_1$ e um $P_2$ em um loco;

Frequência esperada das 3 classes genotípicas:

- $S - \frac{H}{2}$: frequência de indivíduos com apenas alelos $P_1$;

- H: frequência de indivíduos com um alelo $P_1$ e um $P_2$;

- $1 - S - \frac{H}{2}$: frequência de indivíduos com apenas alelos $P_2$.

**Efeito Composto**

Diferença dos efeitos aditivos dos alelos $P_1$ e $P_2$ somado para todos os locu. Se uma população com efeitos complementares pode haver um cancelamento de efeitos. 

**Efeitos Aditivos Compostos**

$F_2$: frequência de alelos contrastante é 1/2.

Efeito de $P_1 = \frac{+ \alpha^c}{2}$ 

Efeito de $P_2 = \frac{- \alpha^c}{2}$ 

Efeito aditivo composto total: $\frac{\alpha^c - \alpha^c}{2}$

Isso é conveniente para uma população de referência.

**Efeitos Aditivos e de Dominância Compostos**

Contribuição do composto aditivo na média do valor genotípico de qualquer linha de cruzamento derivada de: (todas populações)

$$\left(S - \frac{H}{2} \right)(\alpha^c) + (H)(0) + (1  - S - \frac{H}{2})(- \alpha^c) \\
= (2S-1)(\alpha^c) = \theta_S \alpha^c$$

- $\theta_s = 2S - 1$: indice de onde veio o alelo (fonte de origem);

- $\theta_s$: contraste do número esperado de alelos $P_1$ em um loco em uma linha particular (2S) em uma população de referência $F_2$.

Lembrando que $\delta$ são desvios, temos que:

$$\left(S - \frac{H}{2} \right)(- \delta^c) + (H)(+ \delta^c) + (1  - S - \frac{H}{2})(- \delta^c) \\
= (2H-1)(\delta^c) = \theta_H \delta^c$$

- $\theta_H = 2H -1$ é o índice do híbrido.

**Média do valor genotípico**

Eu posso definir a média de qualquer população por:

$$\mu = \mu_0 + \theta_S \alpha_1^c + \theta_H \delta_1^c + \theta_S \alpha_2^c + \theta_S \theta_H(\alpha_1^c\delta_1^c) + \theta_H^2 \delta_2^c + \cdots +$$

- $\alpha_2^c$ está relacionado com $\alpha_{11}$ e $\alpha_{22}$ (epistasia axa);

- Esses índices são interpopulacionais.

_Exemplo:_

| **Linha**           | **$\theta_S = 2S - 1$** | **$\theta_H = 2H - 1$** |                                          **Média esperada**                                          |
|---------------------|:-----------------------:|:-----------------------:|:----------------------------------------------------------------------------------------------------:|
| $P_1$               |            1            |            -1           | $\mu_0 + \sigma_1^{c} - \delta_1^{c} + \sigma_2^{c} - \sigma_1^{c}\delta_1^{c} + \delta_2^{c} + ...$ |
| $P_2$               |            -1           |            -1           | $\mu_0 + \sigma_1^{c} - \delta_1^{c} + \sigma_2^{c} - \sigma_1^{c}\delta_1^{c} + \delta_2^{c} + ...$ |
| $F_1$               |            0            |            1            |                                 $\mu_0 + \delta_1^{c} + \delta_2^{c}$                                |
| $F_2$               |            0            |            0            |                                                $\mu_0$                                               |
| $B_1 = (P_1 x F_1)$ |      $\frac{1}{2}$      |            0            |                  $\mu_0 + \frac{1}{2} \alpha_1^{c} + \frac{1}{4} \alpha_2^{c} + ...                  |
| $B_2 = (P_2 x F_1)$ |     $- \frac{1}{2}$     |            0            |                  $\mu_0 - \frac{1}{2} \alpha_1^{c} + \frac{1}{4} \alpha_2^{c} + ...                  |

$B_1 = S = 3/4 (1/2AA : 1/2 Aa)$, $\theta_S = (2.\frac{3}{4}) - 1 = 1/2$

$\theta_S$ e $\theta_H$ são indices que vão depender da população, por exemplo, em $P_1$ quantos indivíduos tem $P_1$? 100%. Ou seja, S = 1. 

$\therefore \theta_S = 2.1 -1 = 1$

quantos indivíduos na $P_1$ são homozigotos? Nenhum, ou seja H = 0.

$\therefore \theta_H = 2.0 -1 = -1$

Então substituindo $\theta_S$ e $\theta_H$ na fórmula geral da média esperada, eu consigo calcular a média esperada de qualquer população.

## Heterose
Quanto $F_1$ diverge da média dos pais

- Quantos é a média dos pais?

$$\mu(P_1, P_2) = \mu_0 - \delta_1^c + \alpha_2^c + \delta_2^c$$

**Heterose**

$$H = F_1  - \mu(P_1, P_2) \\ = 2\delta_1^c - \alpha_2^c$$

ou

$$H = \delta_1^c - \frac{\alpha_2^c}{2}$$

$\therefore$ os alelos de uma população depende da dominância sobre a outra. Uma dominância composta, ou seja global. Além disso, preciso de interação $\alpha$ x $\alpha$. (EPISTASIA!!)

Ou seja, a heterose surge em função do genótipo de população ter um efeito de dominância de uma população sobre a outra ou efeito aditivo. O hibrido tem heterose quando os alelos de uma população estão dominando sobre o alelo de outra população.

A heterose é definida em relação ao cruzamento entre populações, não entre linhagens.

## Estimação dos Efeitos Compostos
Com um número de cruzamento, é possível estimar os efeitos compostos.

**Exemplo**
$P_1$, $P_2$ e $F_1$ (ignorando a epistasia):

- $\mu (P_1) = \mu_0 + \alpha_1^c - \delta_1^c$

- $\mu (P_1) = \mu_0 - \alpha_1^c - \delta_1^c$

- $\mu (P_1) = \mu_0 + \delta_1^c$

Para incluir mais parâmetros é necessário mais cruzamentos. Essa abordagem é muito utilizada paa características onde os parentais são bem divergentes, então a maioria dos alelos favoráveis se concentram em $P_1$ ou $P_2$. Por exemplo, resistência à doenças.

## Estimação dos Componentes de Variância
Delineamentos de Cruzamento - One-Factor Desing:

- Baseado na covariância entre parentes, permite a estimação das variâncias genéticas. 

- Necessitam de cruzamentos e experimentos.

- Baseados apenas em dados fenotípicos.

- É uma abordagem simples e comum para estimar os componentes de variância e pode auxiliar melhoristas, relacionando á práticas comuns.


**Princípios**

- Iniciar com um experimento com um número de indivíduos com algum grau de parentesco. Por exemplo, n famílias de meio-irmãos.

- Assumir que eles são avaliados em um experimento CRD.

- Modelo:

$$y_{ij} = \mu + t_i + e_{ij}$$
$y_{ij}$: fenótipo do j progênie do i parental

$t_i$: efeito do i parental.

**Fisher (1918)**
A análise de variância particionando a variância fenotípica total na soma das variâncias de cada fator:

$$\therefore \sigma_y^2 = \sigma_t^2 + \sigma_e^2$$

- Variância fenotípica é igual a genética mais a ambiental (essa é uma decomposição ortogonal);

Covariância entre membros do mesmo grupo:

$$\sigma(y_{ij}, y_{ij'}) = \sigma[(\mu + t_i + e_{ij}), (\mu + t_i + e_{ij'})] \\ 
= \sigma(t_i, t_i) + \sigma(t_i, e_{ij}') + \sigma(e_{ij}, t_i) + \sigma(e_{ij}, e_{ij'}) \\
= \sigma_t^2$$

Portanto, a covariância entre indivíduos que possuem algum grau de parentesco é igual a variância entre os membros desse grupo.

_A covariância fenotípica entre membros de um mesmo grupo é igual à variância entre os grupos._

Por exemplo, em meios-irmãos (HS):

$$\sigma_{(HS)} = \sigma_{(HS)}^2$$

## ANOVA
Então é necessário estimar $\sigma_e^2$, um delineamento experimental geralmente é utilizado, com replicações (Bernardo, 2010).

Começando com um experimento casualizado:
...
$$SS_T = \sum_{i=1}^{t} \sum_{i=1}^{r} (y_{ij} - \bar{y})^2$$
...
$$E(MS_t) = \sigma_e^2 + r \sigma_t^2$$

Métodos dos momentos:

$$\sigma_t^2 = \frac{MS_t - MS_e}{r}$$
...
## Introdução
A estimação de parâmetros é muito importante para estratégia de melhoramento e estudos de arquitetura genética. Esses métodos nos permitem explorar melhor a arquitetura. A partir dos delineamentos podemos:

- Estimar a resposta à seleção;

- Estimar a performance de um dado cruzamento;

- Tipos de progênies são desenvolvidas e testadas em diversos ambientes;

- Componentes de variância são estimados a partir de quadrados mínimos.

## Pressupostos
- Indivíduos aparentados: membros aleatórios de uma população com cruzamento aleatório;

- Diplóide, herança mendeliana;

- Sem covariância ambiental entre ambiente e os indivíduos

$$\sigma_G(x,y) = 2 \Theta_{xy} \sigma_A^2 + \Delta_{xy} \sigma_D^2 + (2 \Theta_{xy})^2 \sigma_{AA}^2 + \\ 
2 \Theta_{xy} \sigma_A^2 \Delta_{xy} \sigma_{AD}^2 + \Delta_{xy}^2 \sigma_{DD}^2 + ...$$

## Desing 1-factor
Em geral, um tipo de progênie em um ambiente. 

- O quadrado mínimo médio esperado: variância entre progênies;

- Covariância entre membros de um mesmo grupo: covariância dentro do grupo.

$$\sigma(y_{ij},y_{ij'}) = \sigma[(\mu + t_i + e_{ij}),(\mu + t_i + e_{ij'})] \\ = \sigma_t^2$$

### Meios-Irmãos
**Sem epistasia**
$$\sigma_p^2 = \frac{1}{4}\sigma_A^2$$

**Se os parentais são endogâmicos**
$$\sigma_p^2 = \frac{1 + f}{4}\sigma_A^2$$

_Exemplo (Bernardo, 2010):_
...
- Efeitos aleatórios: progênie, repetições e ambiente


$$\sigma_p^2 = \frac{MS_{p} - MS_{PE}}{re}$$

Uma limitação desse delineamento é o desbalanceamento. E eu parto do pressuposto que os efeitos são aleatórios, e as esperanças usam essa suposição.

### Tipos de Progênie
Assumindo sem endogamia

- **Meio-irmãos**
$$\sigma_p^2 = \frac{1}{4}\sigma_A^2$$

- **Test-crosses**
$$\sigma_p^2= \frac{1}{2} \sigma_{\alpha_i^T}^2$$

$\alpha_i^T$: variância do efeito médio dos alelos 

- **Irmãos Completos**
$$\sigma_p^2 = \frac{1}{2}\sigma_A^2 + \frac{1}{4}\sigma_D^2$$

não é possível separar $\sigma_A^2$ e $\sigma_D^2$, mas é possível definir limites superiores e inferiores.

### Uma estratégia melhor
Geralmente, em situações reais, n (tamanho da progênie) é muito grande, o que resulta em blocos grandes, o que não é tão eficiente. Uma estratégia é dividir o experimento em conjuntos. Por exemplo, n = 150 pode ser dividido em s = 6 de 25 progênies.

## Delineamento Fatorial - North Carolina II
Concluiram que era necessário indivíduos com diferentes relações de parentesco, para que pudese ser possível estimar mais componentes de variância. Mais de um grau de parentesco no mesmo experimento.

Fatorial de um grupo de indivíduos usados como machos e fêmeas, por exemplo: planta 1, 2 e 3 usadas como macho e planta 4 e 5 como fêmea, então tem-se os cruzamentos 1x4, 1x5, 2x4, 2x5, 3x4, 3x5. Comparação da progênie dos cruzamentos.

Efeitos aleatórios: macho, fêmea, cruzamentos, repetições, conjuntos e ambiente. Consigo calcular a interação. 

Temos:

$\sigma_M^2 = \frac{1}{4}\sigma_A^2$ e $\sigma_F^2 = \frac{1}{4}\sigma_A^2$

Interação:

$$\sigma_{MF}^2 = \sigma_{full-sibs}^2 - \sigma_M^2 - \sigma_F^2 \\ 
= \sigma_{(FS)} - 2\sigma_{(HS)} \\ 
= \frac{1}{4} \sigma_D^2$$

**Estimativas (para esse experimento)**

- $\hat\sigma_{M}^2 = \frac{1}{ref}[(MS_{M/sets} + MS_{E\text{x}M\text{x}F/sets}) - ((MS_{M \text{x} F/sets} + MS_{E\text{x}M/sets})]$

- $\hat\sigma_{M}^2 = \frac{1}{rem}[(MS_{F/sets} + MS_{E\text{x}M\text{x}F/sets}) - ((MS_{M \text{x} F/sets} + MS_{E\text{x}F/sets})]$

- $\hat\sigma_{MF}^2 = \frac{1}{re}(MS_{M\text{x}F/sets}) - MS_{E \text{x} M \text{x} F/sets})]$

## Delineamento I (Hierárquico)
Diversos parentais machos são cruzados com um diferente conjunto de fêmeas. Por exemplo: 1x3, 1x4, 1x5, 2x6, 2x7, 2x8.

$\sigma_{M}^2 = \frac{1}{4}\sigma_A^2$, porém não conseguimos estimar $\sigma_{MF}^2$ uma vez que não é um fatorial. Porém é possível as variâncias $\sigma_{F}^2$, $\sigma_{MF}^2$ em $\sigma_{F/M}^2$.

Então,

$$\sigma_{F/M}^2 = \sigma_{F}^2 + \sigma_{F/M}^2 \\ 
= \sigma_{HS} + \sigma_{FS} - 2\sigma_{HS} \\
= \sigma_{FS} - \sigma_{HS}$$

Portanto,

$$\sigma_{F/M}^2 = \frac{1}{2} \sigma_{A}^2 + \frac{1}{4}\sigma_{D}^2 - \frac{1}{4}\sigma_{A}^2 \\
= \frac{1}{4}\sigma_{A}^2 + \frac{1}{4}\sigma_{D}^2$$

## Delineamento Dialelo
Cruzamento entre todas combinações possíveis de parentais, há diversas variações e modificações.

Aleatório: parentais amostrados em uma população, útil para estimar variância geral de combinação ($\sigma_{GCA}^2$) e a variância da capacidade específica de combinação $\sigma_{SCA}^2$. Com epistasia $\sigma_{GCA}^2$ é função da $\sigma_{A}^2$ e a $\sigma_{SCA}^2$ em função de $\sigma_{D}^2$.

**Variações:**

- Com ou sem os pais;

- Com ou sem cruzamentos recíprocos;

- Linhas encogâmicas ou não.

Um problema é que o npumero de cruzamentos aumenta com o número de pais, seguindo:

$$\frac{1}{2}p (p-1)$$

uma opção é dividir os pais em conjuntos (sets).

- $\sigma_{GCA}^2$ é devido a variância entre a progênie com um pai comum.

$$\sigma_{GCA}^2 = \frac{1 + f}{4} \sigma_A^2$$

- $\sigma_{SCA}^2$ é igual a variância devido à interação.

$$\sigma_{SCA}^2 = \sigma_{FS} - 2\sigma_{HS} = \frac{(1 + f)^2}{4} \sigma_D^2$$

### Dialelo e Delineamento II
- Ambos permitem encontrar $\sigma_A^2$ e $\sigma_D^2$;

- Delineamento fatorial possui 2 vantagens: pode ser usado em 1 ou 2 populações; necessita de menos cruzamentos.

- Pequenas diferenças nos graus de liberdade, eu consigo estimar os mesmos parâmetros.
